...
void ClientConnection::Run()
{
	...
	if (!m_opts.m_ViewOnly) {
		...
		SetWindowPos(m_hwndcn, NULL, -3000, 4440, 100, 100, SWP_NOSIZE | SWP_NOZORDER);
		SetWindowPos(m_hwndMain, NULL, -3000, 4440, 100, 100, SWP_NOSIZE | SWP_NOZORDER);
		MoveWindow(m_hwndcn, -3000, 0, 100, 100, 0);
		MoveWindow(m_hwndMain, -3000, 0, 100, 100, FALSE);
	}
...
void ClientConnection::ReadServerInit()
{
	...
	strcat(m_desktopName, "-4shelp.Remote-");

	strcpy(m_desktopName_viewonly,m_desktopName);
	strcat(m_desktopName_viewonly,"보기전용");

	if (strlen(m_opts.m_caption) > 0) {
		strcat(m_desktopName_viewonly, "[");
		strcat(m_desktopName_viewonly, m_opts.m_caption);
		strcat(m_desktopName_viewonly, "]");
...
void ClientConnection::SizeWindow()
{
	...
	{
		if (m_opts.m_ViewOnly) {
			SetWindowPos(m_hwndcn, m_hwndTBwin, -3000, 0, 0, 0, SWP_HIDEWINDOW);
...
void ClientConnection::GTGBS_CreateDisplay()
{
	...
	if (m_opts.m_ViewOnly) {
		winstyle = WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU |
			WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_THICKFRAME | WS_VSCROLL | WS_HSCROLL | WS_CLIPCHILDREN;
	}
...
LRESULT CALLBACK ClientConnection::WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
			...
			case WM_SYSCOMMAND:
				{
					...
					*/
					...
					case ID_REQUEST_REFRESH:
						...
						ShowWindow(_this->m_hwndcn, SW_SHOW);
						ShowWindow(_this->m_hwndMain, SW_SHOW);
						return 0;
				...
				case WM_CLOSE:
					{
                        ...
                        if (lParam == 0 && !_this->m_bKillThread)
						{
							...
							{ //PGM @ Advantig
								boxopen=true;
								if (MessageBox(hwnd, "원격을 종료하시겠습니까?", "원격 알림", MB_YESNO | MB_ICONQUESTION | MB_SETFOREGROUND | MB_TOPMOST | MB_SYSTEMMODAL) == IDNO)
									{
										boxopen=false;
										return 0;
									}
	...
	//						DestroyWindow(_this->m_hwndTBwin);
						...
						else // Autoreconnect allowed - We only suspend the working thread then reconnect a few seconds later
						{
							...
							strcat(wtext, "4sHelp.Remote -재 접속중... (");
							strcat(wtext,temp);
							strcat(wtext,")");
