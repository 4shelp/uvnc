...
void CMenuExecutor::OnTimerEventResolve(ClientConnection*pClient)
{
	...
	int isFileOnOffButton = GetPrivateProfileInt(m_iniKey, MENU_FILETRANSFER_ONOFF, 0, szFileName);
	...
	try {
		...
		if (isFileOnOffButton) {
			SendMessage(pClient->m_hwndMain, WM_SYSCOMMAND, (WPARAM)ID_FILETRANSFER, (LPARAM)0);
			WritePrivateProfileString(m_iniKey, MENU_FILETRANSFER_ONOFF, FUNTION_OFF, szFileName);
			OutputDebugString("isFileOnOffButton on\n");
