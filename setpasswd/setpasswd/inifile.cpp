...
IniFile::ReadPassword(char *value,int valuesize)
{
	...
	//vnclog.Print(LL_INTERR, VNCLOG("%s \n"),myInifilePasswd);
	return GetPrivateProfileStruct("4sRemote","passwd",value,8,myInifile);
}
...
IniFile::WritePassword(char *value)
{
		...
		//vnclog.Print(LL_INTERR, VNCLOG("%s \n"),myInifile);
		return (FALSE != WritePrivateProfileStruct("4sRemote","passwd", value,8,myInifile));
}

bool//PGM
IniFile::ReadPassword2(char *value,int valuesize) //PGM
{ //PGM
	return GetPrivateProfileStruct("4sRemote","passwd2",value,8,myInifile); //PGM
} //PGM

bool //PGM
IniFile::WritePassword2(char *value) //PGM
{ //PGM
		return (FALSE != WritePrivateProfileStruct("4sRemote","passwd2", value,8,myInifile)); //PGM
} //PGM
