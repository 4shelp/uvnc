...
//	RegisterLinkLabel(hInstResDLL);
	...
	strcpy(service_name, "4shelp_remote_service");
	...
	for (i = 0; i < strlen(szCmdLine); i++)
	{
		...
		if (strncmp(&szCmdLine[i], winvncKill, strlen(winvncKill)) == 0)
		{
			...
			hShutdownEventTmp = OpenEvent(EVENT_ALL_ACCESS, FALSE, "Global\\SessionEventUltra3");
...
int WinVNCAppMain()
{
...
#ifdef CRASH_ENABLED
	LPVOID lpvState = Install(NULL,  "rudi.de.vos@skynet.be", "4sRemote");
#endif
	...
	hShutdownEvent = OpenEvent(EVENT_ALL_ACCESS, FALSE, "Global\\SessionEventUltra3");
	hShutdownEventcad = OpenEvent(EVENT_MODIFY_STATE, FALSE, "Global\\SessionEventUltraCad3");
