...
// Constants
const UINT RFB_SCREEN_UPDATE = RegisterWindowMessage("4sRemote.Update.DrawRect");
const UINT RFB_COPYRECT_UPDATE = RegisterWindowMessage("4sRemote.Update.CopyRect");
const UINT RFB_MOUSE_UPDATE = RegisterWindowMessage("4sRemote.Update.Mouse");
const char szDesktopSink[] = "confirmed desktop sink";
...
{	
	...
	if ((GetDeviceCaps(m_hrootdc_Desktop, RASTERCAPS) & RC_BITBLT) == 0)
	{
		MessageBoxSecure(
			...
			"4sRemote cannot be used with this graphic device driver",
			szAppName,
			MB_ICONSTOP | MB_OK
			);
	...
	if ((GetDeviceCaps(m_hmemdc, RASTERCAPS) & RC_DI_BITMAP) == 0)
	{
		MessageBoxSecure(
			...
			"cannot be used with this graphics device driver",
			szAppName,
			MB_ICONSTOP | MB_OK
			);
